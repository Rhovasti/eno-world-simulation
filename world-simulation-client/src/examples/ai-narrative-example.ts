...
import { WorldSimulation, Valley, TimeOfDay } from '../index.js';
...
async function aiNarrativeExample() {
  ...
  try {
    ...
    const themes = ['adventure', 'mystery', 'drama', 'slice-of-life'] as const;
    ...
    const timeNarrative = await simulation.narrative.getTimeSpecificNarrative(
      'Jouy', 
      targetHour, 
      2 // 2 hours of context
    );
...
async function getAIStoryDataImpl(cityName: string) {
