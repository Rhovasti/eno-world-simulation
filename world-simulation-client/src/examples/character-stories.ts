...
async function getCharacterLocation(simulation: WorldSimulation, individual: Individual): Promise<string | null> {
  if (!individual.home_id) return null;
...
export async function getCharacterData(characterId?: number, locationFilter?: string) {
  ...
  try {
    ...
    const characterStories = await Promise.all(
...
function getAgeDistribution(individuals: Individual[]) {
  const ageGroups = { young: 0, adult: 0, senior: 0 };
  
  individuals.forEach(ind => {
    if (ind.age < 30) ageGroups.young++;
    else if (ind.age < 60) ageGroups.adult++;
...
function getActivityDistribution(individuals: Individual[]) {
...
function getWellnessStats(individuals: Individual[]) {
  if (individuals.length === 0) return { avgEnergy: 0, avgHappiness: 0, avgHealth: 0 };
  
  const totals = individuals.reduce(
    (acc, ind) => ({
