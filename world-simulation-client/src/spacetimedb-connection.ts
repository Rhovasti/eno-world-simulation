...
// For demo purposes, we'll create a hybrid approach
// that tries to connect to the real server but falls back to mock data

export interface RealConnectionConfig {
  url: string;
  database: string;
...
export class SpacetimeDBConnection {
  private config: RealConnectionConfig;
  private connected: boolean = false;
  private usingMockData: boolean = false;

  constructor(config: Partial<RealConnectionConfig> = {}) {
    this.config = {
  ...
  async callReducer(name: string, args: any[] = []): Promise<any> {
    ...
    try {
      const response = await fetch(
        `${this.config.url}/v1/database/${this.config.database}/call/${name}`,
        {
          method: 'POST',
  ...
  private getMockData(reducer: string, args: any[]): any {
    switch (reducer) {
