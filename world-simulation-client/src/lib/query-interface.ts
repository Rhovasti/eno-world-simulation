...
export class WorldSimulationClient {
  public spacetimeClient: SpacetimeClient;
   ...
   */
  async getSimulationStatus(): Promise<TimeContext> {
   ...
   */
  async getLocationState(cityName: string, valley?: Valley): Promise<LocationState> {
    const cacheKey = `location_state_${cityName}`;
    const cached = this.getFromCache<LocationState>(cacheKey);
    if (cached !== null) return cached;
  ...
  async getLocationHistory(
    ...
    if (timeRange.event_types && timeRange.event_types.length > 0) {
      filteredEvents = events.filter(event => 
        timeRange.event_types!.includes(event.type)
      );
    }
  ...
  async getIndividualStory(
    ...
    return allEvents.filter(event => 
      event.participants.some(p => p.includes(individualId.toString()))
    );
  }
   ...
   */
  async getCitySummary(cityId: number): Promise<City | null> {
    // Call the reducer
   ...
   */
  async getIndividualsAtLocation(cityName: string): Promise<Individual[]> {
    const cities = await this.spacetimeClient.getCities();
    const city = cities.find(c => c.name.toLowerCase() === cityName.toLowerCase());
    ...
    return individuals.filter(individual => 
      (individual.home_id && cityBuildingIds.has(individual.home_id)) ||
   ...
   */
  async getBuildingsInCity(cityName: string): Promise<Building[]> {
    const cities = await this.spacetimeClient.getCities();
    const city = cities.find(c => c.name.toLowerCase() === cityName.toLowerCase());
  ...
  private calculateTimeOfDay(hour: number): TimeOfDay {
    if (hour >= 5 && hour < 12) return TimeOfDay.Dawn;
    if (hour >= 12 && hour < 17) return TimeOfDay.Day;
    if (hour >= 17 && hour < 21) return TimeOfDay.Dusk;
   ...
   */
  private calculateTimeOfDayForValley(valley: Valley, hour: number): TimeOfDay {
   ...
   */
  private async getRecentEventsForCity(cityId: number, hoursBack: number): Promise<HistoricalEvent[]> {
   ...
   */
  private async getAllRecentEvents(hoursBack: number): Promise<HistoricalEvent[]> {
    const events: HistoricalEvent[] = [];
    const currentHour = await this.getCurrentHour();
    const cutoffHour = currentHour - hoursBack;
