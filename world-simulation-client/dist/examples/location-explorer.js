 ...
 */
import { WorldSimulation, Valley, getCitiesInValley, getLocationStats, getCapitalCities, searchCities, getRandomCity, getRandomCityFromValley, getCityInfo, isValidCity } from '../index.js';
...
export async function getLocationOverview(valleyFilter) {
    ...
    try {
        ...
        const cityStates = await Promise.all(sampleCities.map(city => simulation.client.getLocationState(city)));
        return {
            world_stats: stats,
...
export async function findInterestingLocations(criteria) {
    ...
    try {
        ...
        const cityDetails = await Promise.all(candidateCities.slice(0, 20).map(async (city) => {
...
export { locationExplorerExample };
//# sourceMappingURL=location-explorer.js.map