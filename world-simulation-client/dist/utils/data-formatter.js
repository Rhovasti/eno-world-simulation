 ...
 */
import { TimeOfDay, Valley, IndividualAction, JobType } from '../lib/types';
import { getRelativeTimeDescription } from './time-helpers';
/**
 * Format individual information for narrative use
 */
export function formatIndividual(individual, includeStats = true) {
 ...
 */
export function formatBuilding(building, includeDetails = true) {
 ...
 */
export function formatCity(city, includeMetrics = true) {
 ...
 */
export function formatHistoricalEvent(event, currentHour, includeContext = true) {
 ...
 */
export function formatIndividualActivity(activity) {
    switch (activity) {
...
export function formatBuildingType(buildingType) {
    ...
    if ('Restaurant' in buildingType)
        return 'restaurant';
    if ('Park' in buildingType)
        return 'park';
    if ('Hospital' in buildingType)
        return 'hospital';
    if ('PoliceStation' in buildingType)
        return 'police station';
    if ('School' in buildingType)
        return 'school';
    if ('ResearchLab' in buildingType)
        return 'research laboratory';
    if ('CultureCenter' in buildingType)
        return 'culture center';
    if ('CityHall' in buildingType)
        return 'city hall';
    return 'building';
}
/**
 ...
 */
export function getNeedLevel(value) {
    if (value >= 0.8)
        return 'very high';
    if (value >= 0.6)
        return 'high';
 ...
 */
export function formatWellness(energy, happiness, health) {
 ...
 */
export function formatTimeOfDay(timeOfDay, valley) {
 ...
 */
export function formatValleyDescription(valley) {
    switch (valley) {
        case Valley.Dawn:
 ...
 */
export function createLocationNarrative(cityName, population, timeOfDay, valley, recentEvents, currentHour) {
 ...
 */
export function createCharacterProfile(individual, context) {
 ...
 */
export function formatEventTimeline(events, currentHour, maxEvents = 10) {
    if (events.length === 0) {
...
export function createSummaryStats(individuals, buildings, events) {
    ...
    const totalWellness = individuals.reduce((sum, ind) => sum + (ind.energy + ind.happiness + ind.health) / 3, 0);
 ...
 */
export function formatSummaryStats(stats) {
